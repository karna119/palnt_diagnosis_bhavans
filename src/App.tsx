...
export default function App() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    // Dummy Login: No email, just Name and Phone.
    setTimeout(() => {
      localStorage.setItem('user_details', JSON.stringify(userData));
      localStorage.setItem('expert_session', 'active');
      setIsLoggedIn(true);
      setLoading(false);
      speak(`Welcome ${userData.name}. Institutional access granted.`);
    }, 800);
  };
        ...
        {!openai && (
          <div className="glass-card" style={{ padding: '1.5rem', marginBottom: '2rem', borderColor: '#ff4444', background: 'rgba(255, 68, 68, 0.05)' }}>
            ...
            <p style={{ fontSize: '0.9rem', color: 'var(--text-dim)' }}>
              Diagnosis features are disabled. Please set <strong>VITE_OPENAI_API_KEY</strong> in your environment.
            </p>
          </div>
        )}
